...
<button class="btn btn-success">+ Create</button></form></div>
<h2>All Users</h2><div class="panel"><?php if(empty($data)):?><div class="empty">None</div><?php else:?><table><tr><th>ID</th><th>Username</th><th>Role</th><th>Created</th><th>Last Login</th><th></th></tr><?php foreach($data as $r):?><tr><td>#<?php echo $r['id'];?></td><td><strong><?php echo htmlspecialchars($r['username']);?></strong><?php if($r['id']==$admin_id):?> (you)<?php endif;?></td><td><span class="role <?php echo $r['role'];?>"><?php echo $r['role'];?></span></td><td><?php echo date('M d Y',strtotime($r['created_at']));?></td><td><?php echo $r['last_login']?date('M d H:i',strtotime($r['last_login'])):'Never';?></td><td><?php if($r['id']!=$admin_id):?><form method="POST" style="display:inline" onsubmit="return confirm('Delete?')"><input type="hidden" name="action" value="delete_user"><input type="hidden" name="id" value="<?php echo $r['id'];?>"><button class="btn btn-danger btn-sm">Del</button></form><?php else:?>-<?php endif;?></td></tr><?php endforeach;?></table><?php endif;?></div>
<?php endif;?>
